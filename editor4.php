...
<select id="countries_dropdown" onchange="start(this.value)">
  ...
  <input type="text" id="date-1" name="date-1" placeholder="Date to retrieve">

  <input type="text" id="date-2" name="date-2" placeholder="Date to save to">
  <p style="display:inline-block">Write dates like yyyy-mm-dd</p>
  <br><br>
...
var overlay = L.imageOverlay.rotated("https://upload.wikimedia.org/wikipedia/commons/9/9c/GovernateOfMontenegro.png", topleft, topright, bottomleft, {
    ...
    function start(val) {
      ...
      var datum1 = document.getElementById("date-1").value
      var datum2 = document.getElementById("date-2").value

      country = val;
      console.log(country);

      var geojsonLayer = new L.GeoJSON.AJAX("geojson_files/"+ datum1 +"/" + country + ".geojson");
      ...
      geojsonLayer.on('pm:update', e => {
        ...
        data.append('date', document.getElementById("date-2").value);

        var xhr = new XMLHttpRequest();
        xhr.open('POST', "ajax_store_geojson_after_edit4.php", true);
        xhr.onload = function() {
